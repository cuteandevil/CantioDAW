...
<a href="https://github.com/cuteandevil/CantioDAW/releases">发布</a>
...
**[下载 v0.2.0](https://github.com/cuteandevil/CantioDAW/releases/tag/v0.2.0)** — 一体化包，无需额外下载。

| 文件 | 大小 | 说明 |
|------|------|------|
| CantioDAW.exe | 128 MB | 独立 DAW 桌面应用 (PyQt GUI) |
| cantiodaw-mcp.exe | 88 MB | MCP 服务器 (80 个工具) |
| python_bridge.py | 95 KB | Python 桥接 (56 个 DAW 方法 + Demucs + FluidSynth) |
| demucs.zip | 79 KB | Demucs v4 HTDemucs 音源分离 (35 个 .py) |
| soundfonts.zip | 29.8 MB | GeneralUser GS SoundFont (145 种真实乐器) |
| fluidsynth_dlls.zip | 11 DLLs | FluidSynth 运行时库 |
...
# 或启动 MCP 服务器 (供 AI 代理集成)
cantiodaw-mcp.exe --test    # 自检 (28/28)
cantiodaw-mcp.exe            # 启动 MCP 服务器 (stdio)
cantiodaw-mcp.exe toollist   # 列出全部 80 个工具
```

**依赖:** Python 3.9+，执行 `pip install torch torchaudio soundfile numpy mido scipy`
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| OLLAMA_API_KEY | — | Ollama Cloud 密钥 (LLM 工具必需) |

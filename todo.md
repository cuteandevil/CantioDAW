...
> 构建由 AI Agent 控制的自动编曲系统，基于 CantioDAW 现有代码（ts-orchestrator 29 个 MCP 工具 + Python 核心管线）。
...
└─────────────┬───────────────┘
              ↓
      MIDI / DAW Control      ← 现有 19 个 DAW 工具
              ↓
┌─────────────────────────────┐
...
- [ ] 把现有 29 个工具逐一列出清单：工具名、输入 schema、输出 schema、是否有副作用（修改项目状态 vs 纯查询）
- [ ] 给每个工具打分类标签：`生成类` / `评价类` / `执行类` / `编排类`
- [ ] 标出哪些工具目前是"黑盒"（一次调用做一整套事情）
- [ ] 确认 19 个 DAW 工具和 10 个 LLM 工具之间的共享状态关系
...
- [ ] 定义 `EmotionVector` schema（孤独/希望/怀旧/紧张/平静… 可控维数）
- [ ] 定义 `EnergyCurve` schema（起点/终点/峰值/谷值/曲线形状）
- [ ] 定义 `StyleVector` schema（cinematic / electronic / orchestral / pop / ambient…）
- [ ] 定义 `SceneTags` schema（night / urban / driving / rain / morning / forest…）
- [ ] 定义 `ArrangementSpec` schema（density / instrument_focus / texture）
- [ ] 定义 `StructurePlan` schema（section 列表 + 每段的 IR 引用）

**代码位置**：新建 `cantiodaw/music/ir.py` + `ts-orchestrator/src/music/ir.ts`（双向同步，Python 侧是真理源）
...
| Agent | 职责 | 代码位置 |
|-------|------|---------|
| **Intent Agent** | NL → Music Semantic IR（理解情绪/场景/能量需求） | `ts-orchestrator/src/llm/tools.ts` → 新增 `llm_parse_intent` |
| **Composer Agent** | IR → 曲式结构/旋律设计/和声规划/乐器选择 | `ts-orchestrator/src/orchestrator/workflows.ts` → 新增 `compose_from_ir` 工作流 |
| **Parameter Agent** | IR → 参数 delta（调用具体 adjust_* 工具） | 新建 `cantiodaw/music/parameter_mapper.py` |
| **Critic Agent** | 分析生成结果 → 发现多维度问题 | 见阶段 5 |
...
- [ ] 设计 `llm_parse_intent` 的 prompt 模板（基于现有 LLM router，复用 `gemma4:31b`）
- [ ] 建立意图分类标签体系（推荐：音乐描述分为"情绪/场景/能量/风格/对比"5 类）
- [ ] 建立情绪标签体系（先定 10 个核心情绪，后续可扩展）
- [ ] 建立场景标签体系（night / urban / rain / forest / morning / …）
- [ ] 建立风格标签体系（cinematic / electronic / orchestral / pop / ambient / …）
- [ ] 增量意图：用户说"再悲伤一点" → 在原 IR 上做 `emotion.sadness += 0.2`
- [ ] 在 MCP 暴露 `llm_parse_intent` 工具

**代码位置**：
- Prompt 模板：`ts-orchestrator/src/llm/prompts/intent_parser.ts`
- 工具：`ts-orchestrator/src/llm/tools.ts` 新增
...
- [ ] 曲式结构生成（Intro → Verse → Chorus → Bridge → Outro）
- [ ] 和声规划（调式/和弦进行，基于 emotion 维度和 style 标签）
- [ ] 旋律设计（音高轮廓/节奏型/动机发展）
- [ ] 乐器选择与配器（基于 style 标签 + arrangement density）
- [ ] 整合为可直接执行的"编曲计划"（每个乐器的 MIDI Note 生成方案）
- [ ] 暴露 `llm_compose_from_intent` 工具
...
- [ ] 设计节点 schema（id / label / affects / inverse / related）
- [ ] 建立核心节点（10-15 个音乐表达概念）
- [ ] 实现 `query_graph(concept, direction?) → ParameterDelta[]`
- [ ] 实现反向查询 `reverse_query(target_param) → Concept[]`
- [ ] 图谱作为外部 YAML 文件加载，允许用户自定义
- [ ] 暴露 `llm_query_knowledge_graph` 工具
...
- [ ] 建立 `emotion → harmony` 映射表
- [ ] 建立 `emotion → melody` 映射表（轮廓/音程偏好/方向）
- [ ] 建立 `emotion → rhythm` 映射表（密度/切分/速度）
- [ ] 建立 `emotion → instrument` 映射表（音色/音区/织体）
- [ ] 建立 `emotion → mix` 映射表（混响/压缩/均衡）
- [ ] 映射输出为 `ParameterDelta[]`，直接对应阶段 3 的原子工具参数
...
- [ ] `adjust_pitch(track_id, start, end, delta_cents)` — **已有**
- [ ] `adjust_timing(track_id, start, end, offset_delta_ms)` — **已有**
- [ ] `adjust_volume(track_id, start, end, delta_db)` — **已有**
- [ ] `adjust_dynamics(track_id, section, curve_delta)` — 新增
- [ ] `adjust_articulation(track_id, start, end, style, overlap_delta, attack_delta_ms)` — 新增
- [ ] `adjust_vibrato(track_id, start, end, depth_delta, rate_delta)` — 新增
- [ ] `adjust_micro_timing(track_id, adjustments: [{note_index, offset_delta_ms}])` — 新增
- [ ] `adjust_harmonic_color(section, quality_delta, mode_shift)` — 新增
- [ ] `apply_swing(track_id, ratio)` — 新增
- [ ] `apply_rubato(track_id, curve: [{beat, tempo_factor}])` — 新增
...
- [ ] 和弦功能分析（Tonic / Subdominant / Dominant 分布）
- [ ] 张力曲线分析（dissonance 密度随时间变化）
- [ ] 解决感评分（V→I 占比，延迟解决次数）
- [ ] 离调程度评分
...
- [ ] 动机重复检测（相似音程序列匹配）
- [ ] 旋律发展分析（contour variety / interval variety）
- [ ] 音域分布（过低/过高/最适音域占比）
- [ ] 音程分布分析（大小跳比例）

### 6.3 Rhythm Critic

- [ ] Groove 分析（摇摆量/微时间偏差模式）
- [ ] 密度分析（note density / rest ratio 时间序列）
- [ ] 稳定性分析（速度波动 / downlock 稳定性）

### 6.4 Audio Critic

- [ ] 能量曲线分析（RMS 随时间变化 vs 期望 IR）
- [ ] 频谱分析（高频亮度 / 低频密度 / 中频清晰度）
- [ ] 动态范围分析（crest factor / loudness range）
- [ ] 空间分析（stereo width / reverb tail）
...
- [ ] 为 project / track 设计唯一 ID 体系
- [ ] 所有工具的输入输出改为引用 ID，而非直接传原始波形
- [ ] 实现版本快照机制：每次修改自动生成一个版本
- [ ] 新增 `diff_versions(project_id, v1, v2)` 工具
- [ ] 新增 `rollback_to_version(project_id, version)` 工具
- [ ] 明确 `.cantio` 工程文件是否需要扩展字段来存版本历史
...
- [ ] 解析 Critic 输出 → 生成修改优先级列表
- [ ] 自动选择优化方向（优先修最高 severity 问题）
- [ ] 调用对应 adjust_* 工具执行修改
- [ ] 修改后自动重跑 critic 验证
...
- [ ] 为迭代循环设定上限（单片段最多 5 轮）
- [ ] 设定质量阈值：critic 评分达标即停止
- [ ] "连续 N 轮无改善则停止并上报"规则
- [ ] 每轮迭代前后自动 `diff_versions`，倒退则回滚

### 8.3 双速渲染

- [ ] `render_preview`：低采样率/简化路径，几秒内出结果
- [ ] `render_final`：确认满意后走完整质量渲染
- [ ] 两者接口一致，agent 只传 `"preview"`/`"final"` 参数
...
- [ ] 用户评分接口（1-5 分，关联到版本 ID）
- [ ] A/B 测试支持（两个版本并排对比，用户选择偏好）
- [ ] 重听/收藏行为记录
- [ ] 修改采纳率跟踪（用户接受/拒绝了哪些 critic 建议）

### 9.2 偏好模型

- [ ] 数据标注格式定义
- [ ] Reward Model 训练（基于用户反馈数据）
- [ ] 偏好模型集成到收敛控制（critic 评分中加入偏好加权）
...
- [ ] 设计 `request_checkpoint` 工具
- [ ] checkpoint 返回当前 vs 上一版本的关键指标对比
...
- [ ] 阶段 0 现状盘点完成
- [ ] Music Semantic IR 定义完成（至少包含 emotion + energy + style）
- [ ] `llm_parse_intent` 实现并暴露（Intent Agent 最小版本）
- [ ] 基础 `llm_compose_from_intent` 实现（Composer Agent 最小版本）
- [ ] 验证：输入"一段宁静的钢琴曲" → 产出可播放 MIDI

### 第二阶段（目标：评价闭环）

- [ ] Harmony Critic 最小版本（和弦功能分析 + 张力曲线）
- [ ] Melody Critic 最小版本（动机重复检测 + 音域分析）
- [ ] Critic 统一输出格式实现
...
- [ ] 全部 4 个 Critic 子系统完成
- [ ] Knowledge Graph 完成
- [ ] Parameter Mapping Engine 完成（全 emotion 覆盖）
- [ ] 人类反馈采集系统完成
